...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        elif symbol not in self.bindings and self.parent:
        ...
        if len(formals) != len(vals):
           raise SchemeError
            ...
            while (len(formals) > 0) and (len(vals) > 0):
                Frame.define(child, formals.first, vals.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        Frame.define(env, target, scheme_eval(expressions.second.first, env))
        return target
        # END Question 5A
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN Question 9A
        Frame.define(env, target.first, LambdaProcedure(expressions.first.second, expressions.second, env))
        return target.first
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    check_condition = scheme_eval(expressions.first, env)
    if scheme_true(check_condition):
        return scheme_eval(expressions.second.first, env)
    elif not scheme_true(check_condition) and expressions.second.second:
...
def do_and_form(expressions, env):
    ...
    while len(expressions) > 0:
        check_condition = scheme_eval(expressions.first, env)
        if not scheme_true(check_condition):
            return False
        elif len(expressions.second) == 0:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        args = Pair(scheme_eval(clause.second.first, env), args)
...
def check_formals(formals):
    ...
    formal_list = []
    while len(formals) > 0:
        if (scheme_symbolp(formals.first) == True) and (formals.first not in formal_list):
            formal_list += formals.first
